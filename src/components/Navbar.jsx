...
const Navbar = () => {
  const [isOpen, setIsOpen] = useState(false);
  const location = useLocation(); // It always tracks the current path.

  // No need for separate state for activeLink, use location.pathname directly
          ...
          <ul className="flex flex-col items-center py-4 space-y-4">
            {["/", "/products", "/blogs", "/contact", "/about"].map((path) => (
              <li key={path}>
                <Link
                  to={path}
                  onClick={() => setIsOpen(false)} // Clicking on a menu item will close the menu.
